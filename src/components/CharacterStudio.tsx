...
export function CharacterStudio({ settings, info, connected, ollamaAvailable, automationJobs, onCopilotContext, onCreateTurntable, onNotice }: {
  ...
  const addDetailReference = () => patch({ detailReferences: [...active.detailReferences, { id: createId(), label: '', notes: '' }] })
  ...
  const chooseDetailReferenceImage = async (id: string) => {
    const picked = await window.minimax.chooseMedia('image')
    if (!picked) return
    patchDetailReference(id, { image: { ...picked, kind: 'image', preview: await window.minimax.mediaUrl(picked.path) } })
  }
  ...
  return <><div className="standard-page character-studio">
    ...
    <div className="character-studio-grid">
      ...
      <section className="character-workbench">
        ...
        {settings.characterDetailReferencesEnabled && <section className="character-detail-references" aria-labelledby="character-detail-references-title"><header><div><span><Images size={16} /></span><span><strong id="character-detail-references-title">Character detail references</strong><small>Add an image for any custom-named body area or visual detail that needs extra fidelity. These are appended to the character’s reference budget only while this add-on is enabled.</small></span></div><button type="button" className="secondary-button" onClick={addDetailReference}><Plus size={14} />Add detail</button></header>{active.detailReferences.length ? <div className="character-detail-reference-list">{active.detailReferences.map((detail, index) => <article key={detail.id}><button type="button" className="character-detail-image" onClick={() => void chooseDetailReferenceImage(detail.id)} aria-label={detail.image ? `Replace detail image ${index + 1}` : `Upload detail image ${index + 1}`}>{detail.image?.preview ? <img src={detail.image.preview} alt={detail.label || `Character detail reference ${index + 1}`} /> : <><ImagePlus size={19} /><span>Upload image</span></>}</button><div><label>Detail name or body area<input value={detail.label} onChange={(event) => patchDetailReference(detail.id, { label: event.target.value })} placeholder="Use any name you need" /></label><label>Rendering notes<textarea value={detail.notes} onChange={(event) => patchDetailReference(detail.id, { notes: event.target.value })} placeholder="Optional: describe the detail to preserve" /></label></div><button type="button" className="icon-button" onClick={() => removeDetailReference(detail.id)} aria-label={`Remove detail reference ${index + 1}`}><X size={16} /></button></article>)}</div> : <p className="character-detail-empty">No extra detail references yet. Add only the details that need more precision so the 9-picture render budget stays useful.</p>}</section>}
