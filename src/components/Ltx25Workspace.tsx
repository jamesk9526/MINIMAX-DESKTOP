  ...
  return <div className="create-page ltx-workspace">
    ...
    <div className="workspace-grid">
      <section className="composer-panel">
        ...
        <fieldset className="render-size"><legend>Output size</legend><label>Orientation<select value={orientation} onChange={(event) => { const next = event.target.value as Ltx25Orientation; set('resolution', next === 'square' ? '768x768' : LTX25_RESOLUTIONS[next][7] ?? LTX25_RESOLUTIONS[next][0]) }}><option value="landscape">Landscape</option><option value="portrait">Portrait</option><option value="square">Square</option></select></label><label>Resolution<select value={state.resolution} onChange={(event) => set('resolution', event.target.value)}>{LTX25_RESOLUTIONS[orientation].map((size) => <option key={size}>{size.replace('x', ' × ')} · {ltx25ResolutionLabel(size)}</option>)}</select></label><p className="field-help">32-pixel aligned for LTX‑2.5. Larger HD and 1080p-class canvases use more VRAM; Quality first renders at half size, then uses the official latent 2× refinement pipeline.</p></fieldset>
