...
export function ClipMasterBeta({ clip, settings, onClose, onNotice }: { clip: ClipItem; settings: AppSettings; onClose(): void; onNotice(tone: 'error' | 'success' | 'neutral', text: string): void }) {
  ...
  useEffect(() => {
    let disposed = false
    setBusy('metadata'); setMeta(null); setMetadataError(''); setSavedFrames([]); setOutputFolder(''); setExportResult(null); setBatchFrames([])
  ...
  return <div className="modal-backdrop clip-master-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose() }}>
    <section ref={dialogRef} className="clip-master-modal" role="dialog" aria-modal="true" aria-labelledby="clip-master-title">
      ...
      <div className="clip-master-body">
        ...
        <section className="clip-master-export"><div><strong>Export New Clip</strong><small>Exact frame trim · opens a save dialog · defaults to a versioned name in the Clip Master video folder.</small></div><button className="primary-button" onClick={() => void exportClip()} disabled={!meta || Boolean(busy) || endFrame < startFrame}>{busy === 'export' ? <LoaderCircle className="spin" size={15} /> : <Scissors size={15} />}{busy === 'export' ? 'Exporting…' : 'Export New Clip'}</button></section>
